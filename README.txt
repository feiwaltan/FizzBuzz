...
If you want to change the range you can do that with set methods, as I'm using the get method in the defaultInstance(), so you could run multiple instance of this and with different falues set for first and second, as well as changing of words can be done as well, with its own set method.
...
To run this form the command line make sure you have the correct jdk\bin path in wondows variable, or which ever environment you use.

windows only command:

first create the class
javac -cp "<path-of-lib-jar>" "<path=of=echo.java"
then run the class, probably good idea to cd to that dir
java -cp  "<path-of-lib-jar>";. Echo int1 int2